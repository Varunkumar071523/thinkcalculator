  ...
  return (
    <Link href={category.href} className="group rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
      <Card className="h-full transition-colors group-hover:bg-muted/40">
        ...
        <CardContent><p className="leading-6 text-muted-foreground">{category.description}</p>{category.availability ? <Badge className="mt-4" variant="outline">{category.availability}</Badge> : null}</CardContent>
      </Card>
    </Link>
  )
}
