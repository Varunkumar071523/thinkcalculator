type DonutItem = {
   ...
   * grayscale chart-1/chart-2 tokens so existing callers keep their current look. */
...
export function SimpleDonutChart({ title, items }: { readonly title: string; readonly items: readonly [DonutItem, DonutItem] }) {
  ...
  const firstRing = items[0].ringClass ?? "stroke-chart-1"
  const secondRing = items[1].ringClass ?? "stroke-chart-2"
  return (
    <section aria-labelledby="donut-title">
      ...
      <div className="mt-4 flex flex-col items-center gap-5 sm:flex-row">
        <svg className="size-40 shrink-0 -rotate-90" viewBox="0 0 42 42" role="img" aria-label={description}>
          <circle className={`fill-none ${secondRing}`} cx="21" cy="21" r="15.9155" strokeWidth="8" />
          <circle className={`fill-none ${firstRing}`} cx="21" cy="21" r="15.9155" strokeWidth="8" strokeDasharray={`${firstPercent} ${100 - firstPercent}`} />
        </svg>
