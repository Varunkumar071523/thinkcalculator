...
describe("glossary registry", () => {
  ...
  it("publishes substantive terms and excludes drafts", () => { expect(published).toHaveLength(6); expect(published.map((item) => item.slug)).not.toContain("maturity-value"); published.forEach((item) => expect(item.sections.length).toBeGreaterThanOrEqual(4)) })
