...
describe("glossary registry", () => {
  ...
  it("includes the index and published terms in the sitemap", () => { const urls = sitemap().map((item) => item.url); expect(urls).toContain(createCanonicalUrl("/glossary")); published.forEach((item) => expect(urls).toContain(createCanonicalUrl(item.canonicalPath))); expect(urls).not.toContain(createCanonicalUrl("/glossary/maturity-value")) })
})
