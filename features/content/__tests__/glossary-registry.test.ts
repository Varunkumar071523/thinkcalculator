...
describe("glossary registry", () => {
  ...
  it("publishes substantive terms and excludes drafts", () => { expect(published).toHaveLength(7); expect(published.map((item) => item.slug)).not.toContain("maturity-value"); published.forEach((item) => expect(item.sections.length).toBeGreaterThanOrEqual(4)) })
  ...
  it("has valid live internal relations", () => { published.forEach((item) => [...item.relatedCalculators, ...item.relatedContent].forEach((link) => expect(link.href).toMatch(/^\/(finance|blog|guides)\//))) })
