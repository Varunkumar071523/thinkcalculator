...
describe("editorial content registry", () => {
  it("contains exactly six published items and excludes the draft", () => { expect(published).toHaveLength(6); expect(published.map((item) => item.slug)).not.toContain("choosing-between-fd-and-rd") })
  ...
  it("uses only published content for route generation", () => { expect(getContentByType("blog")).toHaveLength(3); expect(getContentByType("guide")).toHaveLength(3); expect(getContentByType("blog").every((item) => item.status === "published")).toBe(true) })
