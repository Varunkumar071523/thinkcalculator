...
describe("editorial content registry", () => {
  it("contains exactly five published items and excludes the draft", () => { expect(published).toHaveLength(5); expect(published.map((item) => item.slug)).not.toContain("choosing-between-fd-and-rd") })
