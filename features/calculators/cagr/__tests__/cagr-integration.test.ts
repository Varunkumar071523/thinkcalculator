...
describe("CAGR production integration", () => {
  ...
  it("preserves authored CAGR glossary links and the Investing learning path", () => {
    ...
    expect(investing.calculators.map((item) => item.id)).toEqual(["sip-calculator", "step-up-sip-calculator", "swp-calculator", "lumpsum-calculator", "cagr-calculator", "inflation-calculator"])
    expect(investing.glossaryTerms.map((item) => item.id)).toEqual(["glossary-sip", "glossary-step-up-sip", "glossary-swp", "glossary-compounding", "glossary-cagr", "glossary-principal", "glossary-tenure", "glossary-inflation"])
