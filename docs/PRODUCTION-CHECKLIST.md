...
- [x] `npm run test` passes: 955 Vitest tests across 93 files (as of Sprint N — EPF/NPS), plus a 264-test Playwright suite (accessibility, keyboard/focus, zoom/reflow, print, reduced motion) across Chromium, Firefox, and WebKit, after Sprint 31 Browser/Accessibility/Print QA and its follow-up independent review. See section 29 for Sprint N's full-suite Playwright verification (490 tests, run across the whole `tests/e2e/` directory, not just the permanent accessibility/keyboard/zoom/print subset this line originally described).
...
- [x] Sprint 45 added Google Consent Mode v2 (default-denied, `components/analytics/consent-banner.tsx`) and two custom events, `calculation_completed`/`result_shared` — see [DECISIONS.md](DECISIONS.md) #37. The Sprint 36 "no custom events" line above is superseded by that entry, not still current.
- [x] Reviewed: every custom event payload carries only `calculator_type`/`category` identifiers, never a calculator input or result value (structurally gated in `lib/analytics.ts`'s `trackEvent`, verified by `lib/__tests__/analytics.test.ts` and `tests/e2e/analytics.spec.ts` across 3 calculators); no event of any kind fires without an explicit Accept; the privacy policy still names GA4 and what it collects; a non-blocking footer notice links to it; no perf regression since both scripts stay `afterInteractive` only.
- [x] GA4 property created this sprint; Measurement ID `G-EV1LDDV3XG` is documented in [DEPLOYMENT.md](DEPLOYMENT.md)'s "GA4" section as the value to set.
- [ ] Open item: set `NEXT_PUBLIC_GA_MEASUREMENT_ID=G-EV1LDDV3XG` as the GitHub Actions repository secret (GitHub Settings → Secrets and variables → Actions) — not committed to the repo, and not something this environment can do on the project owner's behalf. Not done as part of this sprint's code change.
...
## 29. Sprint N — EPF and NPS bespoke-chart calculators

- [x] Two clean-slate calculators added: EPF (`/finance/epf-calculator`, 3-way stacked-bar corpus breakdown) and NPS (`/finance/nps-calculator`, asset-allocation donut + blended-rate growth line). Both are registered `published` in `calculatorRegistry`, grouped into the existing `savings`/`investments` taxonomy groups (no new group added), and appear on `/finance` and `/calculators` automatically via the existing registry-driven grids — no manual card wiring needed.
- [x] `YearlyBarChart` (`components/calculators/yearly-bar-chart.tsx`) widened from a fixed 2-series stack to a 2-or-3-series union; `SimpleDonutChart` (`components/calculators/simple-donut-chart.tsx`) widened from a fixed 2-item ring to a 2-or-3-item union, with its per-segment arc math generalized to a cumulative-offset technique. See [DECISIONS.md](DECISIONS.md) #26 for the full reasoning, including why no new bespoke chart component was created for either calculator.
- [x] Every existing caller of both widened components (EMI, PPF, retirement corpus, SIP, lumpsum, FD, RD, GST, home loan eligibility, SWP, step-up SIP) was confirmed unaffected — none were modified, and the full pre-existing test suite (`vitest run`) passes unchanged.
- [x] `npm run lint`: clean (0 errors, 0 warnings after fixing one `react-hooks/exhaustive-deps` warning in `nps-calculator.tsx`).
- [x] `next build`: clean, 64 routes generated including `/finance/epf-calculator` and `/finance/nps-calculator`.
- [x] `vitest run`: 955/955 tests passing across 93 files, including new unit tests for `buildStackedDatasets`/`formatCompactINR` (yearly-bar-chart) and `computeDonutSegments` (simple-donut-chart) covering the 2-way, 3-way, zero-total, and 100%-to-one-segment edge cases; new `calculate-epf`/`calculate-nps` tests covering zero contribution, single-year tenure, and 100%-to-one-asset-class allocation; and full schema/URL-state/production-integration coverage for both calculators. Adding two calculators to the registry required bumping the hardcoded "published calculator count" assertion (16 → 18) in eight pre-existing sibling integration tests plus `lib/production-readiness.test.ts` — an expected, mechanical ripple effect of registry growth, not a behavior change in those calculators.
- [x] Full Playwright suite (`node scripts/run-e2e.mjs`, all specs, Chromium/Firefox/WebKit): 488 passed, 5 flaky-then-passed-on-retry, 2 failed — both failures are `print.spec.ts`'s WebKit print-emulation check for **EMI** and **Inflation** (pre-existing calculators, not EPF/NPS). Verified via an isolated `git worktree` at the pre-sprint base commit (`d06338c`) that this exact failure (schedule table reporting `hidden` under WebKit's print media emulation) reproduces identically with zero sprint changes applied — confirmed pre-existing, unrelated to this sprint's `YearlyBarChart`/`SimpleDonutChart` changes, and not fixed here (out of scope; flagged for a future print-QA sprint). EPF and NPS's own pages passed every registry-driven cross-cutting check that runs automatically over all published calculators: the accessibility sweep (`a11y.spec.ts`), zoom/reflow at 375px (`reflow.spec.ts`), and FAQPage structured-data (`structured-data.spec.ts`).
- [x] `tests/e2e/card-links.spec.ts` (the explicit regression this sprint's brief called out): re-run in isolation, confirms every link on `/`, `/finance`, and `/blog` — including the new EPF and NPS cards — resolves via a real HTTP request, not just a rendered href.
- [x] Both calculators include a worked example implementation-checked against their own `calculateEPF`/`calculateNPS` output (`epfWorkedExampleResult`/`npsWorkedExampleResult` tests), matching the AGENTS.md-required worked-example convention.
- [x] `docs/DECISIONS.md` #26 records the component-architecture checkpoint outcome; `PROJECT.md`'s "Completed capabilities" section records the sprint outcome.

## 30. Known e2e flake — Home Loan Eligibility a11y sweep, Firefox (surfaced during the Sprint N Leave Encashment run)

- [ ] `tests/e2e/a11y.spec.ts` — `accessibility sweep › calculator:home-loan-eligibility-calculator — /finance/home-loan-eligibility-calculator` timed out (30,000ms) on the `firefox` project during the full Playwright run for the Leave Encashment calculator sprint, then reported as flaky (passed on the automatic retry). This is a **distinct flake from** the existing WebKit print-emulation flake tracked in section 29, item 227 (EMI/Inflation, `print.spec.ts`) — different spec, different browser project, different failure mode (timeout vs. hidden-element assertion). Do not conflate the two when triaging.
- [x] Confirmed unrelated to the Leave Encashment sprint's changes: `git status` at the time showed no modifications to `home-loan-eligibility-calculator.tsx`, its feature folder, `tests/e2e/a11y.spec.ts`, or any shared a11y-relevant component. Re-running the single test in isolation (`node scripts/run-e2e.mjs --project=firefox -g "calculator:home-loan-eligibility-calculator"`) passed cleanly in 7.7s with no timeout.
- [ ] Not fixed here — out of scope for the Leave Encashment sprint; tracking only. Root cause not investigated (candidate causes: the same worker-count/resource-contention pattern documented in `playwright.config.ts`'s `retries`/`workers` comment for the WebKit print flake, or something specific to this page under Firefox — undetermined).
- [ ] Watch for a pattern: this is now the second distinct flake type surfacing from `test:e2e`'s full 3-browser parallel run as the published-calculator count (and therefore the a11y/reflow/structured-data sweep size) grows. If a third distinct flake appears in a future sprint, that is a signal to investigate `workers`/`retries` tuning or per-suite sharding rather than continuing to log each occurrence individually.

## 31. Second data point on WebKit resource-contention flakiness (surfaced during the Sprint N EPS Pension calculator run)

- [ ] During EPS Pension calculator verification, one full `test:e2e` run (all specs, Chromium/Firefox/WebKit, 20 published calculators) took roughly **34.5 minutes**, about **3x** the ~12-minute duration of an equivalent full run earlier in the same sprint. That run reported 32 failures and 5 flaky-then-passed, almost all WebKit: the accessibility sweep (`a11y.spec.ts`) failed simultaneously for ~20 unrelated, pre-existing calculators (EMI, home-loan-eligibility, SIP, lumpsum, FD, RD, CAGR, PPF, GST, gratuity, HRA, income-tax, SWP, inflation, retirement-corpus, EPF, NPS, leave-encashment, and EPS Pension itself), plus several static/glossary pages, plus `keyboard.spec.ts` for five representative calculators, plus one `structured-data.spec.ts` case (Firefox) and one `print.spec.ts` case (WebKit, the pre-existing EMI/Inflation flake already tracked in section 29).
- [x] Confirmed this was **not** a defect in any specific calculator, EPS Pension included: isolated, single-worker re-runs of `a11y.spec.ts` for `eps-pension-calculator` (twice), `emi-calculator`, and `gratuity-calculator` all passed cleanly on WebKit with no timeout, once run without the other ~500 tests competing for the same machine. This mirrors section 30's finding for the Home Loan Eligibility Firefox flake — the failure is a property of the full parallel run under load, not of the page under test.
- [ ] **This is the third distinct flake type** section 30 flagged as the trigger to stop logging occurrences individually and instead investigate `workers`/`retries` tuning or per-suite sharding in `playwright.config.ts` — following, chronologically: (1) section 29's WebKit `print.spec.ts` EMI/Inflation flake, (2) section 30's Firefox `a11y.spec.ts` Home Loan Eligibility flake, (3) this sprint's mass WebKit `a11y.spec.ts`/`keyboard.spec.ts` failure across ~20 calculators simultaneously, with total run time scaling to ~3x normal. Unlike the first two (a single test, single browser, single page), this occurrence scales with the number of published calculators being swept — each new calculator added to the registry adds its own `a11y.spec.ts`/`reflow.spec.ts`/`keyboard.spec.ts`-eligible test cases, so the parallel-worker resource pressure this pattern depends on grows every sprint a calculator ships, not just occasionally.
- [ ] Not investigated or tuned in this sprint — tracking only, per this sprint's explicit scope. A future sprint should treat this as supporting evidence to prioritize the already-deferred dev-server/Playwright-performance backlog item (worker count, retry budget, and/or splitting the full-suite run into smaller sharded jobs) before the registry grows further and full-suite run time keeps climbing.

## 32. WebKit native date-input a11y timeout — Capital Gains calculator (Sprint N)

- [x] The Capital Gains calculator (`/finance/capital-gains-calculator`) is the site's **first calculator using a native `<input type="date">`** (`components/calculators/calculator-date-input.tsx`, for each purchase lot's date and the sale date). Its own `a11y.spec.ts` accessibility sweep fails on the `webkit` project specifically: `frame.evaluate: Test timeout of 30000ms exceeded` inside axe-core's `analyze()` call, taking ~40-41s against a 30s budget.
- [x] Confirmed **reproducible, not transient flakiness**: re-run in isolation (single test, `--project=webkit --repeat-each=3 --retries=0`, no other tests competing for the machine) failed identically on all 3 repeats (40.5s, 41.1s, 41.2s). This is a different signature from sections 30 and 31's flakes, both of which passed cleanly once isolated from full-suite parallel load — this one does not pass in isolation on WebKit.
- [x] Confirmed **not a real accessibility defect**: the identical test passed cleanly on both other engines with zero violations — `chromium` in 5.2s, `firefox` in 7.3s. Manual dev-server verification (add lot, trigger the conditional FMV field, remove lot, confirm FIFO re-matching recalculates correctly) also showed zero console errors.
- [x] **Conclusion: this is a WebKit native-date-input timeout artifact, not a real accessibility defect.** The page's actual accessibility characteristics are clean (proven by two of three engines); WebKit's handling of `<input type="date">` inside its accessibility tree is the most likely reason axe's evaluation runs long specifically on this page and this engine, in this sandboxed environment.
- [ ] Not fixed here — the shared 30s `a11y.spec.ts` timeout is global test infrastructure affecting every page and was not changed for one calculator; a bespoke non-native date-picker was not built to work around a sandboxed-test-only artifact with no evidence of a real Safari or production impact.

## 33. Sprint 45 — GA4 consent banner and calculator event tracking

- [x] Audited first (per sprint scope): Sprint 36's GA4 scaffolding (`lib/analytics-config.ts`, `components/analytics/google-analytics.tsx`, `tests/e2e/analytics.spec.ts`, footer notice, privacy policy) already existed and was extended, not duplicated. See [DECISIONS.md](DECISIONS.md) #37 for the full decision record.
- [x] Google Consent Mode v2 added: `analytics_storage` defaults to `denied` on every page load before GA4's `config` call runs; a fixed-bottom Accept/Decline banner (`components/analytics/consent-banner.tsx`, equally-weighted buttons, no dark pattern) calls `gtag('consent', 'update', ...)` on Accept and replays a previously-stored "granted" choice on every subsequent page load for returning visitors.
- [x] Two custom events added — `calculation_completed` (debounced 800ms after a calculator's result settles, `features/calculators/core/use-track-calculation.ts`, wired into all 21 published calculators) and `result_shared` (fired from the existing "Copy share link" action, `components/calculators/calculator-actions.tsx`) — both consent-gated structurally in `lib/analytics.ts`'s `trackEvent`, and both payload-limited by TypeScript's `CalculatorEventPayload` type to exactly `{ calculator_type, category }`, with no code path able to add a third field.
- [x] `page_location` query-string stripping (Sprint 36's original leak-prevention mechanism, #25) is unchanged in behavior, now additionally covered by a direct unit test (`stripPageLocationQuery` in `lib/analytics.ts` / `lib/__tests__/analytics.test.ts`) rather than only the existing e2e coverage.
- [x] No-PII verification, per the sprint's explicit requirement: `lib/__tests__/analytics.test.ts` unit-tests the event payload shape for 5 calculators; `tests/e2e/analytics.spec.ts` proves it end-to-end (real hook + component, sentinel input values, asserted absent from `window.dataLayer`) for 3 calculators (EMI, SIP, income tax) plus the `result_shared` share-link path; a fifth new test in the same file confirms zero events fire when consent has not been granted.
- [x] A new, separate, opt-in Playwright suite (`tests/e2e-analytics/`, `npm run test:e2e:analytics` → `scripts/run-e2e-analytics.mjs` + `scripts/serve-static-export.mjs` + `playwright.analytics.config.ts`) builds a real `STATIC_EXPORT=true` export and proves, against the actual served static HTML: the `gtag.js` script tag is present and the consent banner renders; declining consent produces zero GA4 collect requests; accepting consent produces a collect request whose `page_location` has the share-link query string stripped. This is the Node-portable static-export Playwright harness [DEPLOYMENT.md](DEPLOYMENT.md) had flagged as unaddressed follow-up work since Sprint 34 — now built. `playwright.config.ts`/`scripts/run-e2e.mjs` (the main suite, which must never build with GA4 enabled) were not modified.
- [x] Full verification this sprint: `npm run lint` clean, `tsc --noEmit` clean (two pre-existing, unrelated errors in `content-registry.test.ts`/`run-e2e-teardown.test.ts` left untouched — not introduced by this sprint), `vitest run` 1247/1247 passing (120 files), a plain `next build` 67/67 routes, a `STATIC_EXPORT=true` build 67/67 routes, the main Playwright e2e suite passing (analytics.spec.ts: 8/8 including 5 new), and the new `tests/e2e-analytics/` suite 3/3 passing.
- [x] GA4 property created this sprint; Measurement ID `G-EV1LDDV3XG` documented in [DEPLOYMENT.md](DEPLOYMENT.md) as the value to set.
- [ ] Open item (manual, cannot be done from this repository): set `NEXT_PUBLIC_GA_MEASUREMENT_ID=G-EV1LDDV3XG` as a GitHub Actions repository secret.
- [ ] **Forward-looking note**: any future calculator that introduces a date input (native `<input type="date">` or any component built on top of it) should expect this same WebKit timeout signature on its own `a11y.spec.ts` entry and can cross-reference this section rather than re-investigating from scratch — verify the same way this entry did (isolated repeat-run on WebKit to confirm reproducibility, then chromium/firefox to confirm no real violation) before assuming a regression.
- [ ] **Distinct from, not merged into, sections 29/30/31**: those are pre-existing sandboxed-WebKit limitations with different root causes — section 29 is a print-media-emulation rendering assertion (`print.spec.ts`, hidden-element detection), sections 30/31 are parallel-run resource contention that clears on isolated re-run (transient, load-dependent). This section's root cause is native `<input type="date">` accessibility-tree handling under WebKit specifically, and it reproduces even fully isolated — a different mechanism, kept as its own entry rather than folded into any of the three.
