...
**Not run against the export in this sprint.** The existing `playwright.config.ts` starts `next start -p 3104` as its `webServer`, which requires a normal (non-export) `.next` server build — fundamentally incompatible with `output: 'export'`, which produces no server build to start. To make this possible in a future sprint:

1. Add a second Playwright project/config (or an env-conditioned `webServer.command` in the existing config) that serves `out/` with a static file server instead of `next start` — verified in this sprint that this needs to be a server with faithful directory-index and real per-path 404 behavior (the `serve` package's fallback behavior disqualified it; plain `http.server`-style semantics worked correctly here, though a Node equivalent would be needed for CI portability rather than relying on Python).
