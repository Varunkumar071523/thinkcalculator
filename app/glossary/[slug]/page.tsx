...
import { getGlossaryCalculatorBacklinks, getGlossaryStaticParams, getGlossaryTermBySlug } from "@/features/content"
...
export default async function GlossaryTermPage({ params }: Props) {
  ...
  const calculatorBacklinks = getGlossaryCalculatorBacklinks(item.slug)
  return <><GlossaryStructuredData item={item} /><SiteContainer className="py-10 sm:py-14"><nav aria-label="Breadcrumb"><ol className="flex flex-wrap items-center gap-1.5 text-sm text-muted-foreground"><li><Link href="/">Home</Link></li><li aria-hidden><ChevronRight className="size-3.5" /></li><li><Link href="/glossary">Glossary</Link></li><li aria-hidden><ChevronRight className="size-3.5" /></li><li aria-current="page" className="text-foreground">{item.term}</li></ol></nav><div className="mt-8"><article className="max-w-4xl"><header><p className="text-sm font-semibold uppercase tracking-wider text-primary">Financial glossary</p><h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-5xl">{item.term}</h1><p className="mt-5 text-xl leading-8 text-muted-foreground">{item.shortDefinition}</p></header><div className="mt-10 space-y-10">{item.sections.map((section) => <EditorialSection key={section.id} section={section} />)}{calculatorBacklinks.length ? <section aria-labelledby="mentioned-in-calculators"><h2 id="mentioned-in-calculators" className="text-2xl font-semibold tracking-tight">Where this term is used</h2><ul className="mt-4 space-y-2">{calculatorBacklinks.map((link) => <li key={link.href}><Link href={link.href} className="font-medium text-primary underline-offset-4 hover:underline">{link.title}</Link><span className="text-muted-foreground"> — {link.description}</span></li>)}</ul></section> : null}</div></article><ClusterNavigation resourceId={item.id} authoredLinks={authoredLinks} className="mt-14 max-w-4xl" /></div></SiteContainer></>
}
