...
export default async function BlogArticlePage({ params }: Props) { const item = getContentBySlug((await params).slug, "blog"); if (!item || item.status !== "published") notFound(); return <><EditorialStructuredData item={item} /><EditorialLayout item={item} /><RelatedBlogPosts item={item} /></> }
