...
const resources = [
  { title: "Understanding loan EMI", description: "See how principal, interest rate, and tenure shape monthly repayments.", href: "/finance/emi-calculator" },
  { title: "SIP and compounding basics", description: "Explore how regular contributions and time can affect projected growth.", href: "/finance/sip-calculator" },
  { title: "FD vs RD: how they differ", description: "Compare a one-time deposit with regular monthly deposits using both tools.", href: "/finance/fd-calculator" },
]
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="border-y bg-muted/30 py-16 sm:py-20"><SiteContainer><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><SectionHeading eyebrow="Recently added" title="The latest calculators in our library" description="Five production-ready tools for common financial questions." /><Button variant="outline" nativeButton={false} render={<Link href="/finance" />}>View finance tools <ArrowRight aria-hidden="true" /></Button></div><div className="mt-8 divide-y rounded-xl border bg-background">{calculatorCards.map((calculator) => { const Icon = calculator.icon; return <Link key={calculator.href} href={calculator.href} className="group flex items-center gap-4 p-4 hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-muted"><Icon className="size-5" aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block font-medium">{calculator.title}</span><span className="block truncate text-sm text-muted-foreground">{calculator.description}</span></span><ArrowRight className="size-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5" aria-hidden="true" /></Link> })}</div></SiteContainer></section>

      <section className="py-16 sm:py-20"><SiteContainer><SectionHeading eyebrow="Learning resources" title="Build confidence behind the calculation" description="Our guides are in progress. For now, explore the relevant calculator and its visible formula, example, and FAQs." /><div className="mt-8 grid gap-4 md:grid-cols-3">{resources.map((resource) => <Card key={resource.title}><CardHeader><span className="mb-3 flex size-10 items-center justify-center rounded-lg bg-muted"><BookOpen className="size-5" aria-hidden="true" /></span><CardTitle className="text-lg">{resource.title}</CardTitle></CardHeader><CardContent className="flex h-full flex-col"><p className="leading-6 text-muted-foreground">{resource.description}</p><Link href={resource.href} className="mt-5 inline-flex items-center gap-1.5 font-medium underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Explore the calculator <ArrowRight className="size-4" aria-hidden="true" /></Link></CardContent></Card>)}</div></SiteContainer></section>
